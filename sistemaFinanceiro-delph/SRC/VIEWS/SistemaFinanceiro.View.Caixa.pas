...
begin
  ...
  if trim(edtDocumento.Text) = '' then
		begin
			...
			Application.MessageBox('O campo do Documento esta vazio !!','Atenção', MB_OK + MB_ICONWARNING);
			abort;
		end;
	if trim(edtValor.Text) = '' then
		begin
			edtValor.SetFocus;
			Application.MessageBox('O campo de Valor esta vazio !!','Atenção', MB_OK + MB_ICONWARNING);
			abort;
		end;
