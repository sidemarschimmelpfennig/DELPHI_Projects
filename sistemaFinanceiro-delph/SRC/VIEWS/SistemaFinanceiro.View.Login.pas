...
begin
 if Trim(edtLogin.Text) = ''  then
  begin
  edtLogin.setFocus;
  Application.MessageBox('O campo de Login esta Vazio ','atenção' , MB_ICONWARNING + MB_OK);
  abort;
  end;
  if Trim(edtSenha.Text) = ''  then
  begin
  edtSenha.setFocus;
  Application.MessageBox('O campo de Senha esta Vazio ','atenção' , MB_ICONWARNING + MB_OK);
  abort;
  end; 
  try
  ...
  ModalREsult := mrOk;
	  ...
	  begin
		  Application.MessageBox(PWideChar(Erro.Message),'atenção' , MB_ICONWARNING + MB_OK);
		  edtLogin.SetFocus;
	  end;
  end;
end;
